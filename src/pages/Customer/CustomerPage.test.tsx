...
import {
  ...
  createCustomerWithPartialFields,
} from '@/test/testData';
...
describe('CustomerPage', () => {
  ...
  describe('Component rendering with valid customer ID', () => {
    ...
    it('displays customer card when customer data is loaded', async () => {
      ...
      expect(
        within(customerCard).getByText(mockCustomer.storeName)
      ).toBeVisible();
      ...
      expect(
        within(customerCard).getByText(/123 Main St.*Test City.*Test Town/)
      ).toBeVisible();
    ...
    it('displays customer card with missing optional fields', async () => {
      ...
      expect(
        within(customerCard).getByText(customerWithMissingFields.storeName)
      ).toBeVisible();
      expect(
        within(customerCard).getByText(customerWithMissingFields.name!)
      ).toBeVisible();
      // Address is displayed as just the address when locality/town are missing
      expect(
        within(customerCard).getByText(customerWithMissingFields.address)
      ).toBeVisible();
      // Phone number is formatted
      expect(within(customerCard).getByText('(123)-555-0101')).toBeVisible();
      ...
      expect(
        within(customerCard).queryByText('john@test.com')
      ).not.toBeInTheDocument();
      expect(
        within(customerCard).queryByText('(555)-010-2')
      ).not.toBeInTheDocument();
      expect(
        within(customerCard).queryByText('Test City')
      ).not.toBeInTheDocument();
      expect(
        within(customerCard).queryByText('Test Town')
      ).not.toBeInTheDocument();
      expect(
        within(customerCard).queryByText('1.234.567.890')
      ).not.toBeInTheDocument();
    });
    ...
    it('displays customer card with only some optional fields', async () => {
      ...
      // Assert required fields are present
      expect(
        within(customerCard).getByText(customerWithPartialFields.storeName)
      ).toBeVisible();
      expect(
        within(customerCard).getByText(customerWithPartialFields.name!)
      ).toBeVisible();
      // Address is displayed as concatenated string with locality when town is missing
      expect(
        within(customerCard).getByText(/123 Main St.*Test City/)
      ).toBeVisible();
      // Phone number is formatted
      expect(within(customerCard).getByText('(123)-555-0101')).toBeVisible();
      ...
      expect(
        within(customerCard).getByText(customerWithPartialFields.email!)
      ).toBeVisible();
  ...
  describe('Customer not found state', () => {
    it('displays not found message when customer is not found and not loading', async () => {
      ...
      expect(
        screen.getByText(textConstants.misc.CUSTOMER_NOT_FOUND_DESCRIPTION)
      ).toBeVisible();
    });
  });
  ...
  describe('Loading state and data fetching behavior', () => {
    it('shows loading state while fetching customer data', async () => {
      ...
      const customerCard = await screen.findByRole('article');
      expect(
        within(customerCard).getByText(mockCustomers[0].storeName)
      ).toBeVisible();
    });
  });
