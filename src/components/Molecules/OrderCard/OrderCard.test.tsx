...
describe('OrderCard', () => {
  ...
  it('renders single product order correctly', () => {
    testRender(<OrderCard order={mockOrderSingleProduct} products={mockProducts} />);
    ...
    expect(within(singleProductCard).getByText(/Wolf Snack Mix Bolsa/)).toBeVisible();
    ...
    const totalPriceElements = within(singleProductCard).getAllByText(/\$38,97/);
    expect(totalPriceElements.length).toBeGreaterThan(0);
  });

  it('renders navigation header when navigate prop is true', () => {
    testRender(<OrderCard order={mockOrder} products={mockProducts} navigate={true} />);
  ...
  it('renders plain text when navigate prop is false', () => {
    testRender(<OrderCard order={mockOrder} products={mockProducts} navigate={false} />);

    // Check that order ID is displayed as text, not a link
    const title = screen.getByRole('heading', { level: 2, name: /^Pedido/ });
    expect(title).toBeVisible();
    expect(title).toHaveTextContent('12.345');
    expect(screen.queryByRole('link', { name: /pedido #?12\.345/i })).not.toBeInTheDocument();
  });
  ...
  it('renders delete button', () => {
    ...
    const deleteButton = screen.getByRole('button', { name: /eliminar pedido/i });
  ...
  it('shows delete confirmation modal when delete button is clicked', () => {
    ...
    expect(screen.getByText(/Confirmas que deseas eliminar el pedido/i)).toBeVisible();
  ...
  it('applies custom className when provided', () => {
    ...
    testRender(<OrderCard order={mockOrder} products={mockProducts} className={customClass} />);
  ...
  it('displays formatted prices with thousand separators', () => {
    ...
    testRender(<OrderCard order={orderWithHighPrice} products={mockProducts} />);
  ...
  it('displays product total prices when not navigating', () => {
    ...
    const smallPriceMatches = within(card).getAllByText((_, node) => node?.textContent === '$8,5');
  ...
  it('handles missing product information gracefully', () => {
    ...
    testRender(<OrderCard order={orderWithMissingProduct} products={mockProducts} />);
  ...
  it('renders with zero quantity products', () => {
    ...
    testRender(<OrderCard order={orderWithZeroQuantity} products={mockProducts} />);
  ...
  it('displays correct date format', () => {
    ...
    testRender(<OrderCard order={orderWithSpecificDate} products={mockProducts} />);
