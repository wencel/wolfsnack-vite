...
vi.mock('@/components/Atoms/Calendar', () => ({
  default: ({ onChange, value, isRange, maxDate }: any) => (
    <div data-testid="calendar" data-is-range={isRange} data-max-date={maxDate?.toISOString()}>
      <input
        data-testid="calendar-input"
        value={value ? JSON.stringify(value) : ''}
        onChange={(e) => {
          try {
            const parsedValue = e.target.value ? JSON.parse(e.target.value) : undefined;
...
describe('OrderFilterModal', () => {
  ...
  it('initializes calendar with parent date range', () => {
    const testDateRange = [new Date('2024-01-01'), new Date('2024-01-31')] as LooseValue;
  ...
  it('updates calendar when parent date range changes and modal reopens', () => {
    ...
    const newDateRange = [new Date('2024-02-01'), new Date('2024-02-28')] as LooseValue;
    testRerender(
      <OrderFilterModal 
        {...defaultProps} 
  ...
  it('updates date range when calendar value changes', () => {
    ...
    fireEvent.change(calendarInput, { target: { value: JSON.stringify(newDateRange) } });
  ...
  it('sets max date to current date', () => {
    ...
    const calendar = screen.getByTestId('calendar');
    expect(calendar).toHaveAttribute('data-max-date', currentDate.toISOString());
    
    vi.useRealTimers();
  });

  it('resets calendar to parent date range when modal is reopened', () => {
    const initialDateRange = [new Date('2024-01-01'), new Date('2024-01-31')] as LooseValue;
    const propsWithDateRange = {
      ...defaultProps,
      parentDateRange: initialDateRange,
    };
    
    const { testRerender } = testRender(<OrderFilterModal {...propsWithDateRange} />);
    
    // Change calendar value
    const calendarInput = screen.getByTestId('calendar-input');
    const changedDateRange = [new Date('2024-02-01'), new Date('2024-02-28')] as LooseValue;
    fireEvent.change(calendarInput, { target: { value: JSON.stringify(changedDateRange) } });
    
    // Close modal
    testRerender(<OrderFilterModal {...propsWithDateRange} showModal={false} />);
  ...
  it('maintains calendar state during user interactions', () => {
    ...
    fireEvent.change(calendarInput, { target: { value: JSON.stringify(testDateRange) } });
  ...
  it('handles rapid calendar changes correctly', () => {
    ...
    fireEvent.change(calendarInput, { target: { value: JSON.stringify(dateRange1) } });
    fireEvent.change(calendarInput, { target: { value: JSON.stringify(dateRange2) } });
    fireEvent.change(calendarInput, { target: { value: JSON.stringify(dateRange3) } });
