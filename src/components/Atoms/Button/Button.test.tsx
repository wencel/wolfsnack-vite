...
vi.mock('@radix-ui/react-tooltip', () => ({
  ...
  Content: ({ children, className, sideOffset, side }: { children: React.ReactNode; className: string; sideOffset: number; side: string }) => (
    <div data-testid="tooltip-content" className={className} data-side-offset={sideOffset} data-side={side}>
      {children}
    </div>
  ),
...
describe('Button', () => {
  ...
  describe('Loading State', () => {
    ...
    it('uses custom loading text when provided', () => {
      renderButton({ loading: true, loadingText: 'Processing...', children: 'Submit' });
      
      expect(screen.getByText('Processing...')).toBeVisible();
      expect(screen.queryByText(textConstants.misc.SUBMITTING)).not.toBeInTheDocument();
    });
  ...
  describe('Tooltip Functionality', () => {
    ...
    it('removes title attribute when tooltip is provided to avoid duplication', () => {
      renderButton({ tooltip: 'Help text', title: 'Original title', children: 'Button' });
  ...
  describe('Accessibility', () => {
    ...
    it('provides fallback aria-label when no children or tooltip', () => {
      ...
      const button = screen.getByRole('button');
      expect(button).toHaveAttribute('aria-label', textConstants.misc.BUTTON_TEXT);
    });
  });
  ...
  describe('Event Handling', () => {
    ...
    it('calls onClick handler when link is clicked', () => {
      const handleClick = vi.fn();
      renderButton({ href: '/test', onClick: handleClick, children: 'Click me' });
    ...
    it('does not call onClick when button is disabled', () => {
      const handleClick = vi.fn();
      renderButton({ onClick: handleClick, disabled: true, children: 'Click me' });
    ...
    it('does not call onClick when button is loading', () => {
      const handleClick = vi.fn();
      renderButton({ onClick: handleClick, loading: true, children: 'Click me' });
  ...
  describe('Tooltip Integration', () => {
    ...
    it('renders tooltip arrow', () => {
      ...
      expect(screen.getByTestId('tooltip-arrow')).toHaveClass(styles.tooltipArrow);
    });
  });
});
